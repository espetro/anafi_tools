...
class WorldGen:
    def __init__(self, fdir, grid, goal_pos, subj_pos, drone_pos,
        peds_paths=None, tree_pos=None, door_pos=None, wall_pos=None):
        """
        ...
        self.f = NamedTemporaryFile(
            mode="w", suffix=".world", dir=fdir, delete=False
        )
        
        # "Paint" the goal in green and the subject initial position in red
        self.goal_pos = WorldGen.world_pose(goal_pos[0], goal_pos[1])
        self.subj_pos = WorldGen.world_pose(subj_pos[0], subj_pos[1])
        self.drone_pos = WorldGen.world_pose(drone_pos[0], drone_pos[1], 0.1)

        if peds_paths is not None:
            self.peds_poses = []
            # opts = {"pos": (poses, row/col, list of x/y points), "loop": F/T}
            for opts in peds_paths.values():
                init_pos = opts["pos"][0][0]
                end_pos = opts["pos"][0][-1]
                self.peds_poses.append(
                    WorldGen.world_pose(init_pos[0], init_pos[1])
                )
                self.peds_poses.append(
                    WorldGen.world_pose(end_pos[0], end_pos[1])
                )
    ...
    def _tree_tag(self, name, point):
        """Creates a composed tree tag (canopy + trunk)"""
    ...
    def _door_tag(self, name, center):
        """Creates a composed door tag (top door rail + 2 vertical bars)"""
    ...
    def _wall_tag(self, name, points):
        """Creates a composed wall tag (1+ wall boxes)"""
        tag = []
    ...
    def write_to_template(self):
        """Writes a world template to the given temporary file"""
        includes = []
