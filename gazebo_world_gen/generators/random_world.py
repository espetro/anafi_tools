...
class RandomWorld:
    """Generates a random world given the parameters"""
    ...
    def neighbors(shape, pos, rad=1):
        ...
        jumps = [0] + [
            -x for x in range(1, rad+1)] + [x for x in range(1, rad+1)
        ]
        area = list(starmap(
            lambda a, b: (pos[0] + a, pos[1] + b),
            product(jumps, jumps)
        ))
        
        return [
            p for p in area[1:] if (p[0] in range(max_X) and p[1] in range(max_Y))
        ]
    ...
    def group_by_adj(ls, min_length=2, max_length=1000, common_y=False):
        ...
        groups.append((curr_group, idx_x, idx_y))
        return [
            (g,x,y) for (g,x,y) in groups if len(g) in range(min_length, max_length)
        ]
    ...
    def choose_path(empties, shape, min_length=2):
        ...
        for rn in range(shape[0]):
            curr_row_groups = RandomWorld.group_by_adj(
                [(x,y) for (x,y) in empties if x == rn],
                min_length,
                max_length,
                common_y=(False, rn)
            )
        ...
        for cn in range(shape[1]):
            curr_col_groups = RandomWorld.group_by_adj(
                [(x,y) for (x,y) in empties if y == cn],
                min_length,
                max_length,
                common_y=(True, cn)
            )
    ...
    def _place_objs(self, max_objs):
        ...
        for dn in range(door_rng):
            empties = np.argwhere(self.grid == '')
            nm = "D{}".format(dn)
            # places the door in an empty cell with a random direction
            # (only two: North/South and West/East)
            self.doors[nm] = dict(
                pos = empties[randint(0, len(empties) - 1)],
                ori = ["N", "W"][randint(0,1)]
            )
        ...
        for wn in range(wall_rng):
            ...
            nm = "W{}".format(wn)
            self.walls[nm] = dict(
                pos = RandomWorld.choose_path(empties, self.shape)
                # returns (init:line:end)
            )
